...
var RuntimeIDToNameAndStateStr func(runtimeID uint32) (blockName, blockState string, found bool)
...
var aliasStateRuntimeIDs = map[StateHash]uint32{}
...
var RuntimeIDToNameAndSateStrMapping map[uint32][2]string
...
func registerBlockState(s *GeneralBlock) {
	...
	if s.Name == "minecraft:fence" {
		newBlock := &GeneralBlock{
			Name:       fmt.Sprintf("minecraft:%v_fence", s.Properties["wood_type"].(string)),
			Properties: map[string]interface{}{},
			Version:    s.Version,
		}
		registerAliasBlockStates(newBlock)
	} else if s.Name == "minecraft:carpet" {
		newBlock := &GeneralBlock{
			Name:       fmt.Sprintf("minecraft:%v_carpet", s.Properties["color"].(string)),
			Properties: map[string]interface{}{},
			Version:    s.Version,
		}
		if s.Properties["color"].(string) == "silver" {
			newBlock = &GeneralBlock{
				Name:       "minecraft:light_grey_carpet",
				Properties: map[string]interface{}{},
				Version:    s.Version,
			}
		}
		registerAliasBlockStates(newBlock)
	} else if s.Name == "minecraft:coral" {
		if s.Properties["dead_bit"].(uint8) == 0 {
			newBlock := &GeneralBlock{
				Name:       fmt.Sprintf("minecraft:%v_coral", s.Properties["coral_color"].(string)),
				Properties: map[string]interface{}{},
				Version:    s.Version,
			}
			registerAliasBlockStates(newBlock)
			...
			newBlock := &GeneralBlock{
				Name:       fmt.Sprintf("minecraft:dead_%v_coral", s.Properties["coral_color"].(string)),
				Properties: map[string]interface{}{},
				Version:    s.Version,
			}
			registerAliasBlockStates(newBlock)
		}
	} else if s.Name == "minecraft:log" {
		newBlock := &GeneralBlock{
			Name: fmt.Sprintf("minecraft:%v_log", s.Properties["old_log_type"].(string)),
			Properties: map[string]interface{}{
				"pillar_axis": s.Properties["pillar_axis"],
			},
			Version: s.Version,
		}
		registerAliasBlockStates(newBlock)
	} else if s.Name == "minecraft:concrete" {
		newBlock := &GeneralBlock{
			Name:       fmt.Sprintf("minecraft:%v_concrete", s.Properties["color"].(string)),
			Properties: map[string]interface{}{},
			Version:    s.Version,
		}
		registerAliasBlockStates(newBlock)
	} else if s.Name == "minecraft:shulker_box" {
		newBlock := &GeneralBlock{
			Name:       fmt.Sprintf("minecraft:%v_shulker_box", s.Properties["color"].(string)),
			Properties: map[string]interface{}{},
			Version:    s.Version,
		}
		registerAliasBlockStates(newBlock)
	} else if s.Name == "minecraft:wool" {
		newBlock := &GeneralBlock{
			Name:       fmt.Sprintf("minecraft:%v_wool", s.Properties["color"].(string)),
			Properties: map[string]interface{}{},
			Version:    s.Version,
		}
		registerAliasBlockStates(newBlock)
	...
	RuntimeIDToNameAndSateStrMapping[rid] = [2]string{
		strings.TrimPrefix(s.Name, "minecraft:"),
		props,
	}
...
func InitMapping(mappingInData []byte) {
	...
	RuntimeIDToNameAndSateStrMapping = make(map[uint32][2]string)
	...
	RuntimeIDToNameAndStateStr = func(rtid uint32) (blockName, blockState string, found bool) {
		pair, found := RuntimeIDToNameAndSateStrMapping[rtid]
		if found {
	...
	StateToRuntimeID = func(name string, properties map[string]interface{}) (runtimeID uint32, found bool) {
		if rid, ok := stateRuntimeIDs[StateHash{name: name, properties: hashProperties(properties)}]; ok {
			return rid, true
		} else if rid, ok := aliasStateRuntimeIDs[StateHash{name: name, properties: hashProperties(properties)}]; ok {
			return rid, true
		} else {
			return AirRID, false
		}
